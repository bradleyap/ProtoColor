...
class CArrowSpin : public CSpinButtonCtrl
{
// Construction
	int x_coord[10] ;
	int y_coord[10] ;
public:
	int AsIs_Y(int dex);
	int AsIs_X(int dex);
	//CArrowSpin( int *array, int _size);
	BOOL Load_Y_Coords(int nSet)
	{
		if(nSet==1 || nSet==5)
...
;
	int Rotate( int nDegrees,int nSet, int index, BOOL x)
	{
		int coordNew;
		double vcos;		
		double vsin;
